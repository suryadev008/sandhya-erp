...
class StoreOwnerCompanyRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            // Company Info
            'company_name' => 'required|string|max:255|unique:owner_companies',
            'pan_number' => ['required', 'regex:/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/', 'unique:owner_companies'],
            'gstin' => ['required', 'regex:/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/', 'unique:owner_companies'],
